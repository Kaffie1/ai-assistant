...
def handle_todo_list(_match: re.Match[str], ctx: ToolContext) -> str:
    """
    功能：列出当前未完成任务。
...
def handle_todo_done_list(_match: re.Match[str], ctx: ToolContext) -> str:
    """
    功能：列出已完成任务。
    输入：命令匹配结果 `_match`、工具上下文 `ctx`。
    输出：已完成任务文本。
...
def handle_todo_done(match: re.Match[str], ctx: ToolContext) -> str:
    """
    功能：将任务标记为完成。
    ...
    if not fact_id:
        return f"[Todo] done {raw_ref}: not_found"
    ok = ctx.task_store.mark_done(fact_id)
    return f"[Todo] done {fact_id}: {'ok' if ok else 'not_found'}"


def handle_todo_delete(match: re.Match[str], ctx: ToolContext) -> str:
    """
    功能：删除任务。
    输入：命令匹配结果 `match`、工具上下文 `ctx`。
    输出：删除结果文本。
    ...
    fact_id = _resolve_task_id(ctx, raw_ref, status="active")
    if not fact_id:
        return f"[Todo] delete {raw_ref}: not_found"
